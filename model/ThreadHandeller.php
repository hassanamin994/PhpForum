...
class ThreadHandeller extends DBHandeller {
    ...
    function getTree($thread_id){
        $query1 = "select forum.name as forum_name,thread.description as description,thread.id as thread_id,thread.edit_by as edit_by, thread.last_update as last_update, thread.title as thread_title ,user.created_at as userdate,user.role as role,user.id as ownerId,user.username as owner,user.image as image,thread.created_at as date,count(comment.id) as numOfComments FROM "
