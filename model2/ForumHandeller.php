...
class ForumHandeller extends DBHandeller {
    ...
    function getTree($forum_id){
        $query1 = "select forum.name as forum_name,thread.id as thread_id,thread.name as thread_name,thread.title as thread_title ,user.id as ownerId,user.username as owner,user.image as owner_img,user.role as owner_role,thread.created_at as date,count(comment.id) as numOfComments FROM "
