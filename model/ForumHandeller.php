...
class ForumHandeller extends DBHandeller {
    ...
    function getTree($forum_id){
        $query1 = "select forum.name as forum_name, forum.id as forum_id, thread.id as thread_id,thread.title as thread_title, thread.description as thread_content , thread.sticky as stickybit, user.id as ownerId,user.username as owner,user.image as owner_img,user.role as owner_role,thread.created_at as date,count(comment.id) as numOfComments FROM "
